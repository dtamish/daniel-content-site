...
if (app) {
  ...
  function openPdf(concept: Concept) { if (!concept.pdfUrl) return; window.open(concept.pdfUrl, '_blank', 'noopener,noreferrer'); }
  ...
  required<HTMLButtonElement>('[data-change-identity]').addEventListener('click', () => identityDialog.showModal()); menuButton.addEventListener('click', openDrawer); required<HTMLButtonElement>('[data-close-drawer]').addEventListener('click', closeDrawer); scrim.addEventListener('click', closeDrawer); required<HTMLButtonElement>('[data-start-tutorial]').addEventListener('click', () => { welcomeDialog.close(); tutorialDialog.showModal(); }); required<HTMLButtonElement>('[data-skip-tutorial]').addEventListener('click', finishOnboarding); required<HTMLButtonElement>('[data-finish-tutorial]').addEventListener('click', finishOnboarding);
