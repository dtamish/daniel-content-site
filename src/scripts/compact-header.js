...
const labels=()=>{const he=document.documentElement.lang==='he';toggle.textContent=he?'☰ תפריט':'☰ Menu';toggle.setAttribute('aria-label',he?'תפריט חשבון ופעולות':'Account and actions menu');panel.setAttribute('aria-label',he?'חשבון ופעולות':'Account and actions');document.querySelectorAll('[data-view]').forEach(e=>{const label=e.dataset.view==='grid'?(he?'תצוגת רשת':'Grid view'):(he?'תצוגת רשימה':'List view');e.setAttribute('aria-label',label);e.title=label;});};labels();new MutationObserver(labels).observe(document.documentElement,{attributes:true,attributeFilter:['lang']});
